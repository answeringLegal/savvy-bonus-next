...
export default function SettingsPage() {
  ...
  const [localSplits, setLocalSplits] = React.useState(splits);

  const isSplitsInvalid =
    // all splits should add up to 1
    localSplits?.reduce((acc, curr) => acc + curr.percentage, 0) !== 1;
  return (
          ...
          {localSplits?.map((setting) => (
            <div key={setting.ordinal} className='grid grid-cols-4 gap-4 py-4'>
              ...
              <Input
                ...
                max={1}
                className={cn(
                  'border border-foreground/20 rounded-md p-2 text-sm',
                  {
                    'border-red-500':
                      setting.percentage < 0 ||
                      setting.percentage > 1 ||
                      isSplitsInvalid,
                  }
                )}
                ...
                onChange={(e) => {
                  const newSplits = localSplits.map((split) =>
                    split.ordinal === setting.ordinal
                      ? { ...split, percentage: parseFloat(e.target.value) }
                      : split
                  );
                  setLocalSplits(newSplits);
                }}
              />
              <p>{(setting.percentage * 100).toFixed()}%</p>
